...
lexicalAnalyzer text = f (convertToMyString text) 1 [] []
    where 
        ...
        changeState chr@(MyChar val pos) state curStr res
            | state == 1 = case cls of CharClassLetter ->    (2, val:curStr, res)
                                       ...
                                       otherwise ->          (-1, "Illegal character '"++[val]++"' on " ++ (show pos), [])
            | state == 2 = case cls of CharClassLetter -> (2, val:curStr, res)
                                       CharClassDigit -> (2, val:curStr, res)
                                       otherwise -> (changeState chr 1 [] $ (Lexeme (reverse curStr) (findLexemCodeOrID $ reverse curStr) pos):res)
            | cls == CharClassError = (-1, "Illegal character '"++[val]++"' on " ++ (show pos), [])
